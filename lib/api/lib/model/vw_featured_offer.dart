...
class VwFeaturedOffer {
  /// Returns a new [VwFeaturedOffer] instance.
  VwFeaturedOffer({
    this.id,
    this.title,
    this.voucherCode,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is VwFeaturedOffer &&
    other.id == id &&
    other.title == title &&
  ...
  @override
  String toString() => 'VwFeaturedOffer[id=$id, title=$title, voucherCode=$voucherCode, expiry=$expiry, details=$details, description=$description, redemptionPeriod=$redemptionPeriod, howToRedeem=$howToRedeem, isGlobal=$isGlobal, imageUrl=$imageUrl, establishmentId=$establishmentId, baseUrl=$baseUrl, fileName=$fileName, location=$location]';
  ...
  /// Returns a new [VwFeaturedOffer] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static VwFeaturedOffer? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "VwFeaturedOffer[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "VwFeaturedOffer[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return VwFeaturedOffer(
  ...
  static List<VwFeaturedOffer> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <VwFeaturedOffer>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = VwFeaturedOffer.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, VwFeaturedOffer> mapFromJson(dynamic json) {
    final map = <String, VwFeaturedOffer>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = VwFeaturedOffer.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of VwFeaturedOffer-objects as value to a dart map
  static Map<String, List<VwFeaturedOffer>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<VwFeaturedOffer>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = VwFeaturedOffer.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
