  ...
  void fetchData() async {
    {
      ...
      for (api.Friend item in friends) {
        api.User user = (await api.UserApi().userGet(id: 'eq.${item.friendId}', select: 'email,uuid,name,icon_url'))!.first;
                        ...
                        onPressed: () {
                          showDialog(
                              ...
                              builder: (BuildContext context) {
                                return TransferDialog(user: item, storedLiqueur: widget.storedLiqueur,);
                              });
                        },
...
class TransferDialog extends StatefulWidget {
  const TransferDialog({super.key, required this.user, required this.storedLiqueur});
  final api.User user;
  final api.StoredLiqueur storedLiqueur;
  @override
  ...
  Widget build(BuildContext context) {
    ...
    return AlertDialog(
      ...
      content: Column(
        ...
        crossAxisAlignment: page == 0 ? CrossAxisAlignment.center : CrossAxisAlignment.start,
        children: [
          if (page == 0) ...[
